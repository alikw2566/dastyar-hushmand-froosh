...
import { Activity, AudioLines, BarChart3, BrainCircuit, BriefcaseBusiness, Building2, CheckCheck, CircleAlert, ClipboardCheck, Headphones, LayoutDashboard, LogOut, Mail, Menu, MessageSquareText, Moon, Plus, Search, Settings2, Sparkles, Sun, UploadCloud, UserRoundSearch, UsersRound } from "lucide-react";
...
type View = "overview" | "calls" | "reviews" | "customers" | "tasks" | "opportunities" | "team" | "coaching" | "reports" | "search" | "messages" | "settings";
...
const nav: Array<{ id: View; label: string; icon: LucideIcon }> = [
  ...
  { id: "calls", label: "تماس‌ها", icon: Headphones },
  { id: "reviews", label: "صف بازبینی", icon: ClipboardCheck },
  { id: "customers", label: "مشتریان", icon: UserRoundSearch },
  { id: "tasks", label: "پیگیری‌ها", icon: CheckCheck },
  { id: "opportunities", label: "فرصت‌ها و اعتراض‌ها", icon: BriefcaseBusiness },
  { id: "team", label: "تیم فروش", icon: UsersRound },
  { id: "coaching", label: "مربیگری", icon: BrainCircuit },
  ...
  { id: "settings", label: "پنل مدیریت", icon: Settings2 },
];
...
export function SalesDashboard({ currentUser }: { currentUser: CurrentUser }) {
  ...
  const visibleNav = useMemo(() => nav.filter((item) => {
    ...
    return !["reviews", "messages", "settings", "team", "reports"].includes(item.id);
  }), [currentUser.role]);
  ...
  return <div className="live-shell">
    <ConversationField variant="dashboard" />
    <aside className={"live-sidebar " + (menuOpen ? "open" : "")}>
      <div className="live-brand"><span><AudioLines size={23} /></span><div><strong>مکالمه‌بان</strong><small>دستیار هوشمند فروش</small></div></div>
      ...
      <nav>{visibleNav.map((item) => { const Icon = item.icon; return <button key={item.id} className={(view === item.id && !selectedCallId ? "active " : "") + (item.id === "settings" ? "admin-entry" : "")} onClick={() => chooseView(item.id)}><i><Icon size={18} strokeWidth={1.9} /></i>{item.label}{item.id === "settings" && <small>فقط مدیر</small>}{item.id === "tasks" && pendingTasks > 0 && <b>{fa(pendingTasks)}</b>}{item.id === "messages" && waitingMessages > 0 && <b>{fa(waitingMessages)}</b>}</button>; })}</nav>
      <div className="live-account"><span>{currentUser.name.slice(0, 1)}</span><div><strong>{currentUser.name}</strong><small>{currentUser.email}</small></div><a href="/api/auth/logout" title="خروج" aria-label="خروج از حساب"><LogOut size={18} /></a></div>
    </aside>
    {menuOpen && <button className="live-scrim" onClick={() => setMenuOpen(false)} aria-label="بستن منو" />}
    <main className="live-main">
      <header className="live-top"><button className="live-menu" onClick={() => setMenuOpen(true)} aria-label="بازکردن منو"><Menu size={21} /></button><div><strong>{selectedCallId ? "جزئیات تماس" : visibleNav.find((item) => item.id === view)?.label}</strong><small>اطلاعات واقعی فضای کاری شما</small></div><button className="live-theme" onClick={() => setTheme(theme === "light" ? "dark" : "light")} aria-label="تغییر پوسته">{theme === "light" ? <Moon size={19} /> : <Sun size={19} />}</button></header>
      <div className="live-content">
        ...
        {!loading && !error && !selectedCallId && view === "reviews" && <ReviewQueue onOpenCall={openCall} onToast={setToast} />}
...
function StateCard({ title, description, action, tone }: { title: string; description: string; action?: React.ReactNode; tone?: string }) {
  return <section className={"live-state " + (tone ?? "")}><span>{tone === "error" ? <CircleAlert size={21} /> : <Activity size={21} />}</span><h1>{title}</h1><p>{description}</p>{action}</section>;
}
function Overview({ overview, calls, pendingTasks, canUpload, onUpload, onCalls, onOpen }: { overview: OverviewData; calls: CallItem[]; pendingTasks: number; canUpload: boolean; onUpload: () => void; onCalls: () => void; onOpen: (id: string) => void }) {
  return <><div className="live-heading"><div><span><Sparkles size={15} /> مرکز عملیات</span><h1>نمای واقعی عملکرد فروش</h1><p>اعداد این صفحه مستقیماً از تماس‌های فضای کاری محاسبه می‌شوند.</p></div>{canUpload && <button onClick={onUpload}><Plus size={18} /> افزودن تماس</button>}</div>
    <section className="live-metrics"><Metric icon={AudioLines} label="کل تماس‌ها" value={fa(overview.total_calls)} /><Metric icon={CheckCheck} label="تکمیل‌شده" value={fa(overview.completed_calls)} /><Metric icon={Activity} label="امتیاز متوسط" value={overview.average_score == null ? "—" : fa(overview.average_score)} /><Metric icon={BarChart3} label="نرخ تبدیل تأییدشده" value={overview.confirmed_conversion_rate == null ? "—" : fa(overview.confirmed_conversion_rate) + "٪"} /><Metric icon={Mail} label="پیگیری باز" value={fa(pendingTasks)} /></section>
...
function Metric({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value: string }) { return <article><i><Icon size={18} /></i><span>{label}</span><strong>{value}</strong></article>; }
...
function UploadDialog({ currentUser, onClose, onDone }: { currentUser: CurrentUser; onClose: () => void; onDone: () => Promise<void> }) {
  ...
  return <div className="live-modal"><button className="live-modal-bg" onClick={onClose} aria-label="بستن" /><form onSubmit={submit}><header><div><span>تماس جدید</span><h2>آپلود فایل صوتی</h2></div><button type="button" onClick={onClose} aria-label="بستن">×</button></header><input ref={input} type="file" accept="audio/*" hidden onChange={(event) => setFile(event.target.files?.[0] ?? null)} /><button type="button" className="live-drop" onClick={() => input.current?.click()}><UploadCloud size={28} /><strong>{file ? file.name : "انتخاب فایل صوتی"}</strong><small>{file ? fa((file.size / 1024 / 1024).toFixed(1)) + " مگابایت" : "MP3، WAV، M4A یا OGG تا ۵۰۰ مگابایت"}</small></button><label className="live-consent"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} /><span>تأیید می‌کنم رضایت لازم برای ضبط و پردازش این تماس دریافت شده است.</span></label>{error && <p className="live-form-error">{error}</p>}<footer><button type="button" className="secondary" onClick={onClose}>انصراف</button><button disabled={!file || !consent || busy}>{busy ? "در حال ارسال..." : "شروع پردازش"}</button></footer></form></div>;
}
function Empty({ title, description, action }: { title: string; description: string; action?: React.ReactNode }) { return <div className="live-empty"><span><Building2 size={22} /></span><h3>{title}</h3><p>{description}</p>{action}</div>; }
