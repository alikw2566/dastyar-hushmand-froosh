...
2. اجرای تنها `web` با `npm run dev` برای توسعه رابط و ذخیره محلی D1/R2 است. این حالت به‌تنهایی Watcher، Worker، PostgreSQL و پردازش OpenAI را اجرا نمی‌کند و معیار پذیرش پایلوت نیست.

## اجزای اصلی

| جزء | مسئولیت | داده پایدار |
|---|---|---|
| `web` | رابط فارسی RTL، احراز هویت، داشبورد و پنل مدیریت | در اجرای کامل از API؛ در حالت سبک از D1/R2 |
